...
abstract class DisplayNode extends DisplayObject {
	...
	public getChild(i: number): any {
		if (this.children[i]) {
			if (this.children[i].isRemoved) {
				throw new Error(`! ERROR @ DisplayNode.render: ${this.children[i]} at ${i} has been removed!`);
			}

			return this.children[i];
		} else {
			throw new Error(
				`! ERROR @ DisplayNode.getChild(): no children found at index ${i}. Total num of children is: ${this.children.length}`,
			);
		}
	}
	...
	public remove() {
		if (this.isRemoved) {
			throw new Error(`! ERROR @DisplayNode.remove: object has been removed!`);
		}
